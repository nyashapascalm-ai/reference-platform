...
async def admin_partners_overview(user=Depends(require_super_admin)):
    ...
    tot_amount_all = tot_amount_month = 0.0
    ...
    for r in rows:
        ...
        amount_all = round(refs_all * price, 2)
        amount_month = round(refs_month * price, 2)

        tot_refs_all += refs_all; tot_refs_month += refs_month
        tot_amount_all += amount_all; tot_amount_month += amount_month

        partners.append({
            "id": str(r["id"]), "name": r["name"], "contact_email": r["contact_email"],
            "price_per_ref": price, "active": r["active"],
            "all_time":   {"refs": refs_all,   "amount": amount_all},
            "this_month": {"refs": refs_month, "amount": amount_month},
        })

    return {
        "partners": partners,
        "totals": {
            "all_time":   {"refs": tot_refs_all,   "amount": round(tot_amount_all,2)},
            "this_month": {"refs": tot_refs_month, "amount": round(tot_amount_month,2)},
        },
...
async def partner_overview(actor=Depends(require_org_actor)):
    ...
    def block(refs):
        return {"refs": refs, "amount": round(refs * price, 2)}
...
async def admin_setup_partner(partner_id: UUID, body: PartnerSetupIn, user=Depends(require_super_admin)):
    ...
    async with db.pool().acquire() as c:
        ...
        async with c.transaction():
            ...
            # Invite (login) â€” reuse org_invites + token.
