...
# Status values that mean the org cannot take new chargeable actions.
_INACTIVE_STATUSES = {"past_due", "canceled", "cancelled", "unpaid", "incomplete_expired"}


async def assert_active(conn, org_id):
    """Raise if billing is enforced AND the org's subscription is inactive.
    Only blocks NEW chargeable actions; read/download of existing records is never gated."""
    if not enforce():
        return
    row = await get_billing(conn, org_id)
    status = (row["status"] if row else None) or "inactive"
    if status in _INACTIVE_STATUSES or status == "inactive":
        # trialing and active are fine; everything else blocks new actions.
        if status not in ("trialing", "active"):
            from fastapi import HTTPException
            raise HTTPException(
                402,
                "Your subscription is inactive. Update billing to send new requests or invite members. "
