...
export default function Dashboard() {
  ...
  const loadMe = useCallback(async () => {
    try {
      let m = await api('/me');
      // If a member was invited but isn't bound to an org yet (e.g. the email
      // confirmation redirect skipped the invite page), complete acceptance now.
      if (m && !m.org_id && !m.worker_id) {
        let pending = null;
        try { pending = window.localStorage.getItem('reffolio_pending_invite'); } catch (e) {}
        if (pending) {
          try {
            await api(`/invite/${pending}/accept`, { method: 'POST' });
            m = await api('/me');
          } catch (e) { /* invalid/expired invite — fall through to onboarding */ }
          try { window.localStorage.removeItem('reffolio_pending_invite'); } catch (e) {}
        }
      }
      setMe(m);
    }
  ...
  let tiles = [];
  if (me?.org_id) {
    /* hidden: Generate reference (issuer flow) — kept live, removed from nav */
    ...
    if (isAdmin) {
      ...
      nav.push({ id: 'orgsetup', icon: 'shield', label: 'Organisation' });
      nav.push({ id: 'apikeys', icon: 'chart', label: 'API' });
    } else {
      /* member: minimal nav — References only (added below for all members) */
    }
    ...
    if (isAdmin) {
      ...
      tiles.push({ id: 'orgsetup', icon: 'shield', title: 'Organisation setup', desc: 'Set your organisation type and contact details.' });
      tiles.push({ id: 'apikeys', icon: 'chart', title: 'API access', desc: 'Generate keys to connect your own systems.' });
    } else {
      /* member: no admin tiles */
    }
    // member-visible: References (everyone in the org; member's list is scoped server-side)
    nav.push({ id: 'requests', icon: 'ref', label: 'References' });
    tiles.push({ id: 'requests', icon: 'ref', title: 'References', desc: 'Request references and view those you have created.' });
  }
  if (me?.worker_id) {
    /* hidden: My references (worker holder view) — kept live, removed from nav */
  ...
  // Get the app — visible to every signed-in account
  nav.push({ id: 'getapp', icon: 'records', label: 'Get the app' });
  tiles.push({ id: 'getapp', icon: 'records', title: 'Get the app', desc: 'Download the Reffolio desktop app for Windows or Mac.' });
  ...
  function Body() {
    ...
    if (view === 'orgsetup') return <OrgSetupPanel me={me} />;
    if (view === 'getapp') return <GetAppPanel />;
    if (view === 'requests') return <ReferencesPanel me={me} />;
...
function RequestForm({ me, onSent }) {
  ...
  async function send() {
    ...
    try {
      ...
      // upload any staged outgoing documents to the new request
      if (stagedFiles.length && r.request_id) {
        for (const f of stagedFiles) {
          try {
            const fd = new FormData(); fd.append('file', f);
            const token = (await supabase.auth.getSession()).data?.session?.access_token;
            await fetch(`${API}/requests/${r.request_id}/attachments`, {
  ...
  return (
    <div>
      ...
      <input type="file" multiple onChange={(e) => setStagedFiles(Array.from(e.target.files || []))} />
      {stagedFiles.length > 0 && <div className="kv">{stagedFiles.length} file(s) ready: {stagedFiles.map(f => f.name).join(', ')}</div>}
      <p className="muted" style={{ marginTop: 8 }}>The referee completes it on a secure link {'\u2014'} no account needed. The candidate is then asked to consent before the reference is released to you. Any documents you attach (e.g. a job description) are sent with the request.</p>
...
function RequestsList({ mode }) {
  ...
  if (openId) return <ReceivedDetail referenceId={openId} onBack={() => setOpenId(null)} />;
        ...
        <div className="item" key={x.id} style={{ marginTop: 10 }}>
          {mode === 'sent' ? (
            ...
            <>
              ...
              <button style={{ marginTop: 6 }} onClick={() => setOpenId(x.reference_id)}>Open reference</button>
            </>
          )}
        </div>
      ))}
...
function OrgSetupPanel({ me }) {
  ...
  const load = useCallback(async () => {
    try {
      ...
      setBrandColor(d.brand_color || '#6C5CE7');
  ...
  async function uploadLogo(file) {
    ...
    try {
      ...
      const key = `${me.org_id || 'org'}-${Date.now()}.${ext}`;
      ...
      setMsg('Logo uploaded \u2014 remember to save.'); setErr(false);
    } catch (e) { setErr(true); setMsg('Logo upload failed: ' + e.message); }
  ...
  async function save() {
    ...
    try {
      await api('/org/profile', { method: 'POST', body: {
        ...
        contact_email: contactEmail || null, company_address: companyAddress || null,
        brand_color: brandColor || null, logo_url: logoUrl || null,
  ...
  return (
    <div className="card">
      ...
      <textarea value={companyAddress} onChange={(e) => setCompanyAddress(e.target.value)} rows={3} placeholder="e.g. Howcroft House, 919 Bradford Road, Birstall WF17 9JY" />

      <div style={{ height: 1, background: 'var(--line, #e7e9f2)', margin: '18px 0' }} />
      <h2 style={{ fontSize: 18 }}>Branding</h2>
      <p className="muted">Your logo and colour appear on the reference requests you send and the page referees complete.</p>
      <label>Brand colour</label>
      <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
        <input type="color" value={brandColor} onChange={(e) => setBrandColor(e.target.value)}
          style={{ width: 48, height: 38, padding: 2, borderRadius: 8, cursor: 'pointer' }} />
        <input value={brandColor} onChange={(e) => setBrandColor(e.target.value)} style={{ flex: 1 }} placeholder="#6C5CE7" />
      </div>
      <label>Logo</label>
      {logoUrl && <div style={{ margin: '6px 0' }}><img src={logoUrl} alt="logo" style={{ maxHeight: 56, maxWidth: 200, borderRadius: 6 }} /></div>}
      <input type="file" accept="image/*" onChange={(e) => uploadLogo(e.target.files && e.target.files[0])} disabled={uploading} />
      {uploading && <div className="kv">Uploading{'\u2026'}</div>}
      <label>Email signature</label>
      <textarea value={emailSignature} onChange={(e) => setEmailSignature(e.target.value)} rows={3}
        placeholder="e.g. Tendai Mugodi, Registered Manager, For You Social Care" />
...
async function buildReferencePdf(d) {
  const JsPDF = await _loadJsPDF();
  const doc = new JsPDF({ unit: 'mm', format: 'a4' });
  const pageW = 210, pageH = 297, margin = 20, contentW = pageW - margin * 2;
  let y = margin;
  const fields = (d.template && d.template.field_schema && d.template.field_schema.fields) || [];
  ...
  function heading(text, size) { size = size || 13; ensure(10); doc.setFont('helvetica','bold'); doc.setFontSize(size); doc.setTextColor(20,20,40); doc.text(text, margin, y); y += size*0.5+3; }
  function labelValue(label, value) {
    if (value === undefined || value === null || value === '') return;
    doc.setFont('helvetica','bold'); doc.setFontSize(9); doc.setTextColor(90,90,110);
    const L = doc.splitTextToSize(String(label), contentW); ensure(L.length*4+6); doc.text(L, margin, y); y += L.length*4+1;
    doc.setFont('helvetica','normal'); doc.setFontSize(11); doc.setTextColor(20,20,40);
    const V = doc.splitTextToSize(String(value), contentW); ensure(V.length*5+3); doc.text(V, margin, y); y += V.length*5+4;
  }
  function rule() { ensure(6); doc.setDrawColor(225,227,240); doc.line(margin, y, pageW-margin, y); y += 6; }
  function small(text) { doc.setFont('helvetica','normal'); doc.setFontSize(9); doc.setTextColor(100,100,120); const L = doc.splitTextToSize(String(text), contentW); ensure(L.length*4+2); doc.text(L, margin, y); y += L.length*4+2; }
  doc.setFont('helvetica','bold'); doc.setFontSize(18); doc.setTextColor(20,20,40); doc.text('Employment reference', margin, y); y += 9;
  small('Reference number: ' + (d.ref_number || ''));
  small('Candidate: ' + (d.worker_name || ''));
  if (d.template) small('Form: ' + d.template.name);
  if (d.received_at) small('Received: ' + new Date(d.received_at).toLocaleString());
  if (d.referee) { small('Referee: ' + (d.referee.full_name || '') + (d.referee.job_title ? ', ' + d.referee.job_title : '')); small('Referee email: ' + (d.referee.work_email || '') + (d.referee.domain_verified ? ' (domain verified)' : ' (domain unverified)')); }
  y += 2; rule();
  heading('Reference details');
  ordered.forEach((k) => labelValue(labelFor(k), (d.content || {})[k]));
  if (d.attachments && d.attachments.length) { rule(); heading('Attachments'); d.attachments.forEach((a) => small((a.direction === 'returned' ? 'From referee: ' : 'Sent with request: ') + a.filename)); }
  rule(); heading('Verification & audit trail');
  small('Tamper-evident hash: ' + (d.content_hash || ''));
  small('Consent: ' + (d.consent_status || ''));
  (d.events || []).forEach((e) => small(e.event_type + (e.actor_name ? ' - ' + e.actor_name : '') + (e.created_at ? ' . ' + new Date(e.created_at).toLocaleString() : '')));
  y += 2; small('This reference is verified and tamper-evident, with a full audit trail. Reffolio reference ' + (d.ref_number || '') + '.');
  doc.save('Reffolio-' + (d.ref_number || 'reference') + '.pdf');
}
...
function ReceivedDetail({ referenceId, onBack }) {
  ...
  async function savePdf() {
    try { await buildReferencePdf(d); }
    catch (e) { setErr('PDF export failed: ' + (e && e.message ? e.message : 'unknown error')); }
  }
